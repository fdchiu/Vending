...
- (void)viewDidLoad {
    ...
    else //leave a 50 pixel space for page counter and settings button
    ...
    //used to show activity when getting random number from web
    [self.activityView setHidesWhenStopped:YES];
    [self.activityView stopAnimating];
    
    //kick off process to get the random numbers
...
//
// getting  new sets of random number
//
- (IBAction)refresh:(id)sender {
    ...
    NSLog(@"RootViewController: refresh data");

}

#pragma mark - datacontroller delegate
// a machine's new set of 3 random data received now update the screen
-(void)updatedFor:(NSString*)machinID
{
    DataViewController *currentView = [self.pageViewController.viewControllers objectAtIndex:0];
    
    if([[[currentView dataObject] valueForKey:@"id"] isEqualToString:machinID])
        [currentView refresh];
    
}

//
//called when all machines random numbers updated
// update page control
//
...
{
    Reachability *r = [Reachability reachabilityWithHostName:@"www.random.org"];
    ...
    if(internetStatus == NotReachable)
    {
        if(TRUE) {
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"No Internet Connection"
                                                                                     message:@"Cannot reach www.random.org"
